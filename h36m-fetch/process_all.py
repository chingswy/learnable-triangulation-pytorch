...
from metadata import load_h36m_metadata, process_camera
...
def process_view(out_dir, subject, action, subaction, camera):
    ...
    pose2dname = path.join(subj_dir, 'MyPoseFeatures', 'D2_Positions', base_filename + '.cdf')
    assert os.path.exists(pose2dname), pose2dname
    with pycdf.CDF(pose2dname) as cdf:
        poses_2d = np.array(cdf['Pose'])
        poses_2d = poses_2d.reshape(poses_2d.shape[1], 32, 2)
    if False:
        pose3dname = path.join(subj_dir, 'MyPoseFeatures', 'D3_Positions_mono_universal', base_filename + '.cdf')
        pose3dname = path.join(subj_dir, 'MyPoseFeatures', 'D3_Positions', base_filename.split('.')[0] + '.cdf')
        assert os.path.exists(pose3dname), pose3dname
        with pycdf.CDF(pose3dname) as cdf:
            poses_3d_univ = np.array(cdf['Pose'])
            poses_3d_univ = poses_3d_univ.reshape(poses_3d_univ.shape[1], 32, 3)
            poses_3d_univ_old = poses_3d_univ
    posename = path.join(subj_dir, 'MyPoseFeatures', 'D3_Positions_mono', base_filename + '.cdf')
    assert os.path.exists(posename), posename
    with pycdf.CDF(posename) as cdf:
        poses_3d = np.array(cdf['Pose'])
        poses_3d = poses_3d.reshape(poses_3d.shape[1], 32, 3)

    cameras = process_camera('extracted/metadata.xml', subject, camera)
    Sall_mono = np.reshape(poses_3d, [-1,3])
    Sall = (cameras['R'].T @ (Sall_mono.T - cameras['T'])).T
    poses_3d_univ = np.reshape(Sall, poses_3d.shape)
    if False:
        diff = np.abs(poses_3d_univ - poses_3d_univ_old)
        print(diff.max())
